...
def export_facts_csv(filename, chartgroup=None, indicatorgroup=None, indicator=None):
    ...
    query = sql.SQL("""
        ...
        WHERE EXISTS (
            ...
            INNER JOIN charts_chartgroup_indicator_groups
                ON charts_chartgroup.id =
                   charts_chartgroup_indicator_groups.chartgroup_id
            INNER JOIN core_indicatorgroup
                ON charts_chartgroup_indicator_groups.indicatorgroup_id =
                   core_indicatorgroup.id
            INNER JOIN core_indicatorgrouplink
                ON (
                    core_indicatorgroup.id =
                    core_indicatorgrouplink.group_id
                    AND core_indicatorgrouplink.indicator_id =
                    core_indicator.id
                )
            WHERE {}
        )
    ...
    copy_query = sql.SQL("COPY ({}) TO STDOUT WITH CSV HEADER").format(query)

    # FileResponse will automatically close this file (and hence be removed)
    tmpf = tempfile.NamedTemporaryFile(prefix="chart-group-export-", mode="wb+")  # noqa: SIM115

    try:
        with connection.cursor() as cursor:
            query_interpolated = cursor.mogrify(copy_query, params)
